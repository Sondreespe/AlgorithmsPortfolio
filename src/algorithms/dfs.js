...
function* dfsGenerator({ nodes, edges, startNode }) {
  ...
  yield {
    ...
    current: null,
    examining: null,
    description: `Initialize: push start node ${startNode} onto stack. Stack: [${stack}].`,
  };
  ...
  while (stack.length > 0) {
    ...
    yield {
      ...
      current,
      examining: null,
      description: `Pop ${current} from stack (LIFO). Stack: [${stack.join(', ') || '—'}].`,
    };
    ...
    for (const neighbor of adj[current]) {
      if (!visited.has(neighbor)) {
        yield {
          ...
          current,
          examining: neighbor,
          description: `Neighbor ${neighbor} of ${current} is unvisited — push onto stack.`,
        };
        ...
        yield {
          ...
          current,
          examining: neighbor,
          description: `${neighbor} marked visited and pushed. Stack: [${stack.join(', ')}].`,
        };
      } else {
        yield {
          visited: new Set(visited),
  ...
  yield {
    ...
    current: null,
    examining: null,
    description: `DFS complete. Visited all ${visited.size} reachable nodes.`,
  };
}

export function generateDFSSteps(input) {
  const steps = [];
  for (const step of dfsGenerator(input)) steps.push(step);
  return steps;
}

export const DEFAULT_DFS_GRAPH = {
  startNode: 0,
...
export function randomizeDFSGraph() {
  const n = 7;
